...
def roofline_model_datasets(**kwargs) -> dict:
    ...
    for name, config in kwargs.items():
        C_CL = config['query_batch_size'] *config['N'] / config['nlist'] * (3 * config['dim'] / 16 - 2 + log2(config['nprobe']))
        IO_CL = config['dim'] * config['query_batch_size'] * 4 + config['N'] / config['nlist'] * (config['dim'] * 8)
        C_RC = config['query_batch_size'] * config['nprobe'] * config['dim'] / 16
        IO_RC = (config['query_batch_size'] + config['nprobe'] ) * config['dim'] * 4
        C_LC = config['query_batch_size'] * config['nprobe'] * config['CB'] * config['dim'] * (3 * config['M'] - 1) / config['M'] / 16
        IO_LC = (4 + config['M'] * config['CB'] * 4)* config['query_batch_size'] * config['nprobe'] + config['dim'] * config['CB'] * 4
        C_DC = config['query_batch_size'] * config['nprobe'] * config['nlist'] * (config['M'] - 1) / 16
