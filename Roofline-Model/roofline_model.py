...
def roofline_model_datasets(**kwargs) -> dict:
    ...
    for name, config in kwargs.items():
        C_CL = config['query_batch_size'] * config['nlist'] * ((3 * config['dim'] - 1) + (ceil(log2(config['nprobe'] + 1)) - 1 + 1))  # CL: L2 distance calculation (substraction, multiplication and accumulation); top-k sorting with a priority queue
        IO_CL = config['query_batch_size'] * (config['nlist'] * config['dim'] * (4 + 4) + 8 * config['nprobe'])  # 4 + 4: query loading, centroid loading; 8 * : sizeof(element(priority_queue))
        C_RC = config['query_batch_size'] * config['nprobe'] * config['dim']  # RC: L1 distance calculation between each pair of query and centroid
        IO_RC = config['query_batch_size'] * config['nprobe'] * config['dim'] * (4 + 4 + 4)  # 4 + 4 + 4: query loading, centroid loading and residual saving
        C_LC = config['query_batch_size'] * config['nprobe'] * config['CB'] * config['M'] * (3 * ceil(config['dim'] / config['M']) - 1)  # LC: L2 distance calculation (substraction, multiplication and accumulation) between each pair of M-dim residual and codebook, and the amount of pairs is |queryBatchSize| * |nprobe| * CB * M
        IO_LC = config['query_batch_size'] * config['nprobe'] * config['CB'] * (config['dim'] * 4 + config['dim'] * 4 + config['M'] * 4)  # Load: residual (once for each codebook entry), codebook; save: LUT
        C_DC = config['query_batch_size'] * config['nprobe'] * ceil(config['N'] / config['nlist']) * (config['M'] + (config['M'] - 1))  # DC (indirect addressing + accumulation): sub-distance accumulation based on the distance LUT for each cluster point in the nprobe closest clusters; each partial distance is fetched before accumulation due to indirect addressing by identified cluster points
        IO_DC = min(config['N'], config['query_batch_size'] * config['nprobe'] * ceil(config['N'] / config['nlist'])) * (config['M'] * ceil(log2(config['CB']) / 8)) + config['query_batch_size'] * config['nprobe'] * (config['M'] * config['CB'] * 4 + ceil(config['N'] / config['nlist']) * 4)  # Load: top-nprobe clusters, LUT (accessed randomly and frequently => always be buffered); save: distances
        C_TS = config['query_batch_size'] * config['nprobe'] * ceil(config['N'] / config['nlist']) * (ceil(log2(config['K'] + 1)) - 1 + 1)  # TS: top-k sorting with a priority queue for each cluster point in the nprobe closest clusters
        IO_TS = config['query_batch_size'] * config['K'] * 8  # top-k saving
